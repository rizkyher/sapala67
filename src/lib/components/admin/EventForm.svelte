<script lang="ts">
	...
	let {
		...
		submitLabel = 'Simpan',
		action = ''
	}: { values?: Partial<EventItem>; error?: string; submitLabel?: string; action?: string } = $props();
...
<form method="POST" enctype="multipart/form-data" {action} class="space-y-5 rounded-2xl border border-sapala-ink/10 bg-white p-4 shadow-card sm:p-6">
	...
	<div class="grid gap-4 sm:grid-cols-2">
		<div>
			<label class={label} for="slug">Slug <span class="font-normal text-sapala-ink/40">(opsional)</span></label>
		...
		<div>
			...
			<input id="category" name="category" value={values.category ?? ''} placeholder="Diklat / Ekspedisi / Pengabdian" class={input} />
		</div>
	</div>
	...
	<div class="grid gap-4 sm:grid-cols-2">
		<div>
			...
			<input id="start_date" name="start_date" type="date" value={values.start_date ?? ''} class={input} />
		</div>
		<div>
			<label class={label} for="end_date">Tanggal Selesai <span class="font-normal text-sapala-ink/40">(opsional)</span></label>
			<input id="end_date" name="end_date" type="date" value={values.end_date ?? ''} class={input} />
		</div>
	</div>

	<div>
		<label class={label} for="location">Lokasi</label>
		<input id="location" name="location" value={values.location ?? ''} class={input} />
	</div>

	<div>
		<label class={label} for="upload">Upload gambar kegiatan</label>
	...
	<div>
		<label class={label} for="description">Ringkasan</label>
		<textarea id="description" name="description" rows="2" class="{input} resize-none">{values.description ?? ''}</textarea>
	</div>

	<div>
		<label class={label} for="content">Detail <span class="font-normal text-sapala-ink/40">(pisahkan paragraf dengan baris kosong)</span></label>
		<textarea id="content" name="content" rows="8" class="{input} resize-y">{values.content ?? ''}</textarea>
	</div>
	...
	<div class="flex flex-col gap-2 pt-2 sm:flex-row sm:gap-3">
		...
		<a href="/admin/events" class="btn-outline justify-center text-sm">Batal</a>
	</div>
</form>
